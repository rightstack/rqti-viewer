...
function normalizeMathBlankAnswer(value: string): string {
...
export function getInputBlankStateClass(options: {
  ...
  if (options.answerKey) return "";

  if (options.isSubmit && options.correctAnswer !== undefined) {
    const ok =
      normalizeMathBlankAnswer(options.value) === normalizeMathBlankAnswer(options.correctAnswer);
    return ok ? "qti-ext-text-entry-input-correct" : "qti-ext-text-entry-input-incorrect";
  }

  return options.value !== "" ? "qti-ext-text-entry-input-focus" : "";
}
