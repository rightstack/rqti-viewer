 ...
 * `transform: scale(availableWidth / designWidth)`을 적용한다. 스케일은 원본
 * 이상(1)으로 올라가지 않으며, 레이아웃 박스 크기에 영향을 주지 않으므로
 * 바깥 래퍼 높이를 `naturalHeight * scale`로 잡아 주변 흐름이 깨지지 않게 한다.
 */
export function FixedScaleContainer({
  ...
  const update = useCallback(() => {
    ...
    if (availWidth > 0 && designWidth > 0) {
      // 원본(designWidth) 이상으로는 확대하지 않는다. 부모 폭이 더 넓으면 1로 고정.
      const next = Math.min(availWidth / designWidth, 1);
